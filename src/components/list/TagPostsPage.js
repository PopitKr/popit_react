...
export default class TagPostsPage extends React.Component {
  ...
  componentDidMount () {
    if (!this.state.posts) {
      this.getTagPosts(this.props.match.params.tag)
    }
  }

  getTagPosts() {
    this.page++;
    PostApi.getPostsByTag(this.props.match.params.tag, [], this.page, MAX_NUM_POSTS)
      .then(json => {
    ...
    return (
      <Layout className="layout" hasSider={false} style={{background: '#ffffff'}}>
        ...
        <Content style={{padding: '20px 10px', maxWidth: 1360, margin: 'auto auto'}}>
          <div style={{width: 900}}>
            <h1>{this.props.match.params.tag.toUpperCase()}</h1>
